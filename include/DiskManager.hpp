...
struct FileList {
  ...
  void link_file_to_page(const int32_t fd, const int32_t page_id) {
    fp_pair[page_id] = fd;
  }
...
struct DiskManager {
  ...
  int32_t lru_replacement(const PageType page_type);
  void	  handle_io();

  [[nodiscard]] PageHandler*       create_page(const int32_t fd,
  ...
  [[nodiscard]] PageHandler* get_page(const int32_t  page_id,
				      const PageType page_type);
private:
  DiskManager();
